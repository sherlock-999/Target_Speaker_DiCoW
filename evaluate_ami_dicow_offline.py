...
def load_ami_reference_rows(
    annotations_root: Path, meeting_id: str, gap_threshold: float = 0.8
):
    rows = []
    ...
    for wf in word_files:
        # AMI filename pattern: MEETING.SPK.words.xml, e.g., EN2002a.A.words.xml
        parts = wf.name.split(".")
        if len(parts) < 3:
            continue
        speaker = parts[1]
        ...
        words = []
        for elem in root:
            if xml_localname(elem.tag) != "w":
                continue
            if str(elem.attrib.get("punc", "")).lower() == "true":
                continue
            text = clean_text(unescape((elem.text or "").strip()))
            if not text:
                continue
            try:
                st = float(elem.attrib["starttime"])
                et = float(elem.attrib["endtime"])
            except Exception:
                continue
            words.append((st, et, text))

        if not words:
            continue

        words.sort(key=lambda x: x[0])
        cur_start, cur_end = words[0][0], words[0][1]
        cur_tokens = [words[0][2]]
        for st, et, tok in words[1:]:
            if st - cur_end > gap_threshold:
                text = clean_text(" ".join(cur_tokens))
                if text:
                    rows.append(
                        {
...
def main():
    ...
    parser.add_argument(
        "--dataset-root",
        type=Path,
        default=Path("/home3/adnan/data/TS_ASR/AMI/eval_ami_full"),
    )
    parser.add_argument(
        "--annotations-root",
        type=Path,
        default=Path("/home3/adnan/data/TS_ASR/AMI/ami_manual_1.6.1"),
    )
    parser.add_argument("--output-dir", type=Path, default=Path("output/ami_dicow_offline"))
    ...
    audio_files = sorted(args.dataset_root.glob("*/audio/*.wav"))
    if len(audio_files) == 0:
        raise RuntimeError(f"No AMI audio files found under: {args.dataset_root}")
    ...
    for idx, wav_path in enumerate(audio_files, start=1):
        session_id = wav_path.parents[1].name
        if session_id in completed_hyp_sessions:
            continue

        ref_multi_rows = load_ami_reference_rows(args.annotations_root, session_id)
        if not ref_multi_rows:
            with failures.open("a", encoding="utf-8") as f:
                f.write(f"{session_id}\tNoReference\tNo AMI words.xml reference rows found\n")
            continue

        ref_single_rows = []
        for seg_i, item in enumerate(ref_multi_rows):
