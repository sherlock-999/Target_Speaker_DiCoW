...
class DiCoWGenerationMixin(WhisperForConditionalGeneration):
    ...
    def generate(
        ...
        pad_kwargs = {"padding": "right"}
        if "device" in inspect.signature(_pad_to_max_length).parameters:
            pad_kwargs["device"] = self.device
        sequences = _pad_to_max_length(
            final_segments, generation_config.pad_token_id, **pad_kwargs
        )
    ...
    def _retrieve_logit_processors(self, generation_config, logits_processor, begin_index, is_shortform, num_beams,
        ...
        if generation_config.suppress_tokens is not None:
            if "device" in inspect.signature(SuppressTokensLogitsProcessor.__init__).parameters:
        ...
        if generation_config.begin_suppress_tokens is not None:
            if "device" in inspect.signature(SuppressTokensAtBeginLogitsProcessor.__init__).parameters:
