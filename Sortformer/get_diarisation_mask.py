...
def main(cfg: DiarizationConfig) -> Union[DiarizationConfig]:
    ...
    #################################################
    # Added Section for generating Diarisation Mask #
    #################################################
    ...
    os.makedirs(cfg.output_diarisation_mask_dir, exist_ok=True)

    for audio_file, diarization_output in all_audiofiles_diarisation_output:
        print("Processing:", audio_file)

        # Convert pyannote object to per-speaker results
        per_speaker_samples = []
        for speaker in diarization_output.labels():
            per_speaker_samples.append(diarization_output.label_timeline(speaker))
            print(speaker)
            print(diarization_output.label_timeline(speaker))
        print("per_speaker_samples:", per_speaker_samples)
        print(len(per_speaker_samples))

        # Create diarization mask
        audio_length = round(diarization_output.get_timeline().extent().end * 50)  # frames at 50 fps
        ...
        print(f"Diarization mask shape: {diarization_mask.shape}")
        print(f"Mask (first 10 frames for all speakers):\n{diarization_mask[:, :10]}")
        
        # Save mask as .pt
        audio_basename = os.path.basename(audio_file).replace(".wav", "")
        mask_path = os.path.join(cfg.output_diarisation_mask_dir, f"{audio_basename}_mask.pt")
        torch.save(diarization_mask, mask_path)
        print(f"Saved diarization mask to {mask_path}")
